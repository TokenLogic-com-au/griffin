...
contract GasBenchmarkTest is Test {
    ...
    uint256 constant LIQUIDITY = 100_000_000 * 1e18;
    uint256 constant TEST_AMOUNT = 10_000 * 1e6;
    ...
    function test_gas_swapToGHO() public {
        usdc.mint(address(this), TEST_AMOUNT);
        usdc.approve(address(router), TEST_AMOUNT);

        uint256 gasBefore = gasleft();
        router.swapToGHO(address(usdc), TEST_AMOUNT, 0);
        uint256 gasUsed = gasBefore - gasleft();

        console2.log("swapToGHO gas used:", gasUsed);

        // Reasonable upper bound for a swap operation
        assertLt(gasUsed, 300_000, "swapToGHO should use less than 300k gas");
    }

    function test_gas_swapFromGHO() public {
        uint256 ghoAmount = TEST_AMOUNT * 1e12; // Convert to 18 decimals

        gho.mint(address(this), ghoAmount);
        gho.approve(address(router), ghoAmount);

        uint256 gasBefore = gasleft();
        router.swapFromGHO(address(usdc), ghoAmount, 0);
        uint256 gasUsed = gasBefore - gasleft();

        console2.log("swapFromGHO gas used:", gasUsed);

        assertLt(gasUsed, 350_000, "swapFromGHO should use less than 350k gas");
    }

    function test_gas_previewSwapToGHO() public view {
        uint256 gasBefore = gasleft();
        router.previewSwapToGHO(address(usdc), TEST_AMOUNT);
        uint256 gasUsed = gasBefore - gasleft();

        console2.log("previewSwapToGHO gas used:", gasUsed);

        // Preview should be much cheaper (view function)
        assertLt(gasUsed, 50_000, "previewSwapToGHO should use less than 50k gas");
    }

    function test_gas_previewSwapFromGHO() public view {
        uint256 ghoAmount = TEST_AMOUNT * 1e12;

        uint256 gasBefore = gasleft();
        router.previewSwapFromGHO(address(usdc), ghoAmount);
        uint256 gasUsed = gasBefore - gasleft();

        console2.log("previewSwapFromGHO gas used:", gasUsed);

        assertLt(gasUsed, 50_000, "previewSwapFromGHO should use less than 50k gas");
    }

    function test_gas_setTokenConfig() public {
        address newToken = makeAddr("newToken");
        address newStata = makeAddr("newStata");
        address newGsm = makeAddr("newGsm");

        uint256 gasBefore = gasleft();
        router.setTokenConfig(newToken, newStata, newGsm);
        uint256 gasUsed = gasBefore - gasleft();

        console2.log("setTokenConfig gas used:", gasUsed);

        assertLt(gasUsed, 100_000, "setTokenConfig should use less than 100k gas");
    }

    function test_gas_comparison_coldVsWarm() public {
        // First swap (cold storage)
        usdc.mint(address(this), TEST_AMOUNT * 2);
        usdc.approve(address(router), TEST_AMOUNT * 2);

        uint256 gasBefore1 = gasleft();
        router.swapToGHO(address(usdc), TEST_AMOUNT, 0);
        uint256 coldGas = gasBefore1 - gasleft();

        // Second swap (warm storage)
        uint256 gasBefore2 = gasleft();
        router.swapToGHO(address(usdc), TEST_AMOUNT, 0);
        uint256 warmGas = gasBefore2 - gasleft();

        console2.log("Cold storage swap gas:", coldGas);
        console2.log("Warm storage swap gas:", warmGas);
        console2.log("Gas savings:", coldGas - warmGas);

        // Warm should be cheaper
        assertLt(warmGas, coldGas, "Warm storage should be cheaper");
    }
}
